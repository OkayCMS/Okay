...
<h1 class="h1">Ваш заказ №{$order->id}
...
<table id="purchases">
    ...
    <tr class="total">
    	<th colspan="5">
		    Итого: <span>{$order->total_price|convert}&nbsp;{$currency->sign}</span>
    	</th>
    </tr>
...
{* Детали заказа *}
<div class="block_heading">Детали заказа</div>
...
{if !$order->paid}
    ...
    {if $payment_methods && !$payment_method && $order->total_price>0}
        <form method="post">
        <div class="block_heading">Выберите способ оплаты</div>
        <ul id="deliveries">
            {foreach $payment_methods as $payment_method}
            	<li>
        			...
        			<label for=payment_{$payment_method->id}>	{$payment_method->name}, к оплате {$order->total_price|convert:$payment_method->currency_id}&nbsp;{$all_currencies[$payment_method->currency_id]->sign}</label>
    ...
    {elseif $payment_method}
        <div class="block_heading">Способ оплаты &mdash; {$payment_method->name}</div>
        <form method=post>
        	<input class="button" type=submit name='reset_payment_method' value='Выбрать другой способ оплаты'>
        </form>
