{if $products|count > 0}
    <div class="sort{if $ajax} is_ajax{/if}">
        ...
        <a {if $sort=='rating'} class="active_up" {elseif $sort=='rating_desc'}class="active_down"{/if} {if $sort=='rating'}href="{furl sort=rating_desc page=null}" {else}href="{furl sort=rating page=null}{/if}" data-language="{$translate_id['products_by_rating']}">{$lang->products_by_rating}</a>
    </div>
{/if}