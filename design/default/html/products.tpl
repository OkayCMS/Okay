...
{* Заголовок страницы *}
{if $keyword}
    <h1 class="page_title">{$lang->poisk} {$keyword|escape}</h1>
{elseif $page}
...
{if $products}
    ...
    <div class="sort">
	    {$lang->sortirovat_po}
    	<a {if $sort=='position'} class="selected"{/if} href="{furl sort=position page=null}">{$lang->umolchaniyu}</a>
    	<a {if $sort=='price'}    class="selected"{/if} href="{furl sort=price page=null}">{$lang->tsene}</a>
    	<a {if $sort=='name'}     class="selected"{/if} href="{furl sort=name page=null}">{$lang->nazvaniyu}</a>
    </div>
    {/if}
