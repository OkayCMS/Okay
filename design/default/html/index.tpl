		...
		<div id="header_top">
			<div class="container">
				...
				<div id="user_block">
					<!-- Обратный звонок -->
					<div class="call_wrap">
						<a class="callback_link" href="#callback_form">{$lang->obratnyj_zvonok}</a>
					</div>
					...
					<div id="account">
						{if $user}
							...
							<a id="logout" href="{$lang_link}user/logout">{$lang->vyjti}</a>
						{else}
							<a id="register" href="{$lang_link}user/register">{$lang->registratsiya}</a>
							<a id="login" href="{$lang_link}user/login">{$lang->vhod}</a>
						{/if}
					</div>
			...
			<div id="header_center" class="clearfix">
				...
				<div id="search">
					<form action="{$lang_link}all-products">
						<input class="input_search" type="text" name="keyword" value="{$keyword|escape}" placeholder="{$lang->poisk_tovara}"/>
			...
			<div class="container">
				<div class="row">
					<div class="advantage_1 column col_4">
						...
						<span class="advantage_name">{$lang->besplatnaya_dostavka}</span>
						<p>{$lang->besplatnaya_dostavka_za_zakaz_svyshe_1000_rub}</p>
					</div>
					<div class="advantage_2 column col_4">
						<div class="a_icon"></div>
						<span class="advantage_name">{$lang->subbotnie_skidki}</span>
						<p>{$lang->ekonomte_do_30_na_subbotnih_rasprodazhah}</p>
					</div>
					<div class="advantage_3 column col_4">
						<div class="a_icon"></div>
						<span class="advantage_name">{$lang->podarok_za_zakaz}</span>
						<p>{$lang->poluchajte_podarok_za_zakaz_svyshe_3000rub}</p>
					</div>
				</div>
			</div>
		</div>
	...
	<footer id="footer">
		<div class="container">
			...
			<div id="subscribe_container">
				<span class="subscribe_heading">{$lang->hotite_podpisatsya_na_nashu_rassylku}</span>
	            <form method="post">
	                <input type="hidden" name="subscribe" value="1" />
	                <input class="subscribe_input" type="text" name="subscribe_email" value="" data-format="email" data-notice="{$lang->vvedite_email}" placeholder="{$lang->vvedite_email}"/>
	                <input class="button" type="submit" value="{$lang->podpisatsya}"/>

	                {if $subscribe_error}
    	            	<div class="message_error">
    	            		{if $subscribe_error == 'email_exist'}{$lang->vy_uzhe_podpisany}{/if}
    	            		{if $subscribe_error == 'empty_email'}{$lang->vvedite_email}{/if}
    	            	</div>
	            	{/if}
	                {if $subscribe_success}
    	                <script>
    	                    ...
    	                    $(function() {
    	                        alert('{$lang->vy_byli_uspeshno_podpisany}');
    	                    });
    	                </script>
	                {/if}
