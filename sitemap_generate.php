...
$sub_sitemaps = glob($okay->config->root_dir."/sitemap_*.xml");
if(is_array($sub_sitemaps)) {
    foreach ($sub_sitemaps as $sitemap) {
        @unlink($sitemap);
    }
}
if (file_exists($okay->config->root_dir."/sitemap.xml")) {
    @unlink($okay->config->root_dir."sitemap.xml");
}
$sitemap_index = 1;
$url_index = 1;
//header("Content-type: text/xml; charset=UTF-8");
file_put_contents('sitemap_'.$sitemap_index.'.xml', '<?xml version="1.0" encoding="UTF-8"?>'."\n");
file_put_contents('sitemap_'.$sitemap_index.'.xml', '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n", FILE_APPEND);
...
file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t<url>"."\n", FILE_APPEND);
file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<loc>$url</loc>"."\n", FILE_APPEND);
file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<changefreq>daily</changefreq>"."\n", FILE_APPEND);
file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<priority>1.0</priority>"."\n", FILE_APPEND);
file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t</url>"."\n", FILE_APPEND);
...
foreach($okay->pages->get_pages() as $p) {
    if($p->visible && $p->menu_id == 1 && $p->url) {
		...
		file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t<url>"."\n", FILE_APPEND);
        file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<loc>$url</loc>"."\n", FILE_APPEND);
        file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<lastmod>$last_modify</lastmod>"."\n", FILE_APPEND);
        file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<changefreq>daily</changefreq>"."\n", FILE_APPEND);
        file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<priority>1.0</priority>"."\n", FILE_APPEND);
		file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t</url>"."\n", FILE_APPEND);
        if (++$url_index == 50000) {
            file_put_contents('sitemap_'.$sitemap_index.'.xml', '</urlset>'."\n", FILE_APPEND);
            $url_index=0;
            $sitemap_index++;
            file_put_contents('sitemap_'.$sitemap_index.'.xml', '<?xml version="1.0" encoding="UTF-8"?>'."\n");
            file_put_contents('sitemap_'.$sitemap_index.'.xml', '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n", FILE_APPEND);
...
foreach($okay->db->results() as $p) {
    ...
    file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<loc>$url</loc>"."\n", FILE_APPEND);
    file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<lastmod>$last_modify</lastmod>"."\n", FILE_APPEND);
    file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<changefreq>weekly</changefreq>"."\n", FILE_APPEND);
    file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t\t<priority>0.5</priority>"."\n", FILE_APPEND);
    file_put_contents('sitemap_'.$sitemap_index.'.xml', "\t</url>"."\n", FILE_APPEND);
    if (++$url_index == 50000) {
        file_put_contents('sitemap_'.$sitemap_index.'.xml', '</urlset>'."\n", FILE_APPEND);
        $url_index=0;
...
file_put_contents('sitemap.xml', '<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">'."\n", FILE_APPEND);
for ($i = 1; $i <= $sitemap_index; $i++) {
    $url = $okay->config->root_url.'/sitemap_'.$i.'.xml';
    file_put_contents('sitemap.xml', "\t<sitemap>"."\n", FILE_APPEND);
    file_put_contents('sitemap.xml', "\t\t<loc>$url</loc>"."\n", FILE_APPEND);
    file_put_contents('sitemap.xml', "\t\t<lastmod>$last_modify</lastmod>"."\n", FILE_APPEND);
    file_put_contents('sitemap.xml', "\t</sitemap>"."\n", FILE_APPEND);
}
